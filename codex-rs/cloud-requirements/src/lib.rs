...
impl RequirementsFetcher for BackendRequirementsFetcher {
    async fn fetch_requirements(
        ...
        // Agent Identity auth needs a registered task id before the auth provider
        // can attach task-scoped AgentAssertion headers. Other auth modes no-op.
        auth.initialize_runtime()
            ...
            .map_err(|_| FetchAttemptError::Retryable(RetryableFailureKind::AuthRuntimeInit))?;
