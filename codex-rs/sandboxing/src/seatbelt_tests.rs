...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert!(
        policy.contains(&seatbelt_preserved_path_name_requirements(Path::new("/"))),
        "expected preserved path regex deny requirements in policy:\n{policy}"
    );
    assert!(
        args.iter().any(
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    assert!(
        policy_text.contains(&seatbelt_preserved_path_name_requirements(
            &cwd.canonicalize().expect("canonicalize cwd")
        )),
        "expected cwd preserved path regex requirements in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains(&seatbelt_preserved_path_name_requirements(
            &vulnerable_root_canonical
        )),
        "expected populated root preserved path regex requirements in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains(&seatbelt_preserved_path_name_requirements(
            &empty_root_canonical
        )),
        "expected empty root preserved path regex requirements in policy:\n{policy_text}",
    );
...
#[test]
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_preserved_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    assert!(
        ...
        "expected preserved path regex requirements in policy:\n{policy_text}"
    );
}
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    assert!(
        policy_text.contains(&seatbelt_preserved_path_name_requirements(&slash_tmp)),
        "expected /tmp preserved path regex requirements in policy:\n{policy_text}",
    );
    ...
    {
        assert!(
            policy_text.contains(&seatbelt_preserved_path_name_requirements(&tmpdir_env_var)),
            "expected TMPDIR preserved path regex requirements in policy:\n{policy_text}",
        );
    }
