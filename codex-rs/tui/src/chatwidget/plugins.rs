...
impl ChatWidget {
    ...
    fn marketplace_remove_confirmation_popup_params(
        ...
        let marketplace_name_for_remove = marketplace_name.clone();
        let marketplace_display_name_for_remove = marketplace_display_name.clone();
                ...
                SelectionItem {
                    ...
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::OpenMarketplaceRemoveLoading {
                            marketplace_display_name: marketplace_display_name_for_remove.clone(),
                        });
                        tx.send(AppEvent::FetchMarketplaceRemove {
                            cwd: cwd_for_remove.clone(),
                            marketplace_name: marketplace_name_for_remove.clone(),
                            marketplace_display_name: marketplace_display_name_for_remove.clone(),
                        });
                    })],
                    ..Default::default()
    ...
    fn plugins_popup_params(
        ...
        for (marketplace, label) in additional_marketplaces.into_iter().zip(labels) {
            ...
            tabs.push(SelectionTab {
                ...
                label: label.clone(),
                header,
                footer_hint: removable_marketplace.then(|| plugins_popup_hint_line(true)),
                items: self.plugin_selection_items(
                    entries,
                    /*include_marketplace_names*/ false,
        ...
        SelectionViewParams {
            view_id: Some(PLUGINS_SELECTION_VIEW_ID),
            header: Box::new(()),
            footer_hint: Some(plugins_popup_hint_line(false)),
            tabs,
...
fn plugins_popup_hint_line(can_remove_marketplace: bool) -> Line<'static> {
    if can_remove_marketplace {
        Line::from(
            "space enable/disable · ←/→ select marketplace · enter view details · r remove marketplace · esc close",
        )
    } else {
